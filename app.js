...
router.route('/resavepw').post(function(req, res) {
   ...
   var paramId = req.body.id || req.query.id;
    var paramPassword = req.body.password || req.query.password;
    var paramPasswordCheck = req.body.passwordcheck || req.query.passwordcheck;
...
router.route('/signup').post(function(req, res) {
   console.log('/signup 호출됨.');
    ...
    var paramPersonalagree = req.body.personalagree || req.query.personalagree;
...
// 사용자id를 인증하는 함수
var loginId = function(database, id, callback) {
    console.log('loginId 호출됨 : ' + id);
 ...
 // 사용자pw를 인증하는 함수
var loginPw = function(database, id, password, callback) {
    console.log('loginPw 호출됨 : ' + id + ', ' + password);
...
// ID찾기 사용자를 인증하는 함수
var findId = function(database, name, phonenum, email, callback) {
    console.log('authUser 호출됨 : ' + name + ', ' + phonenum+', '+email);
    
     // 이름, 전화번호, 이메일을 이용해 검색
    UserModel.find({"name":name, "phonenum":phonenum, "email":email}, function(err, results) {
       if (err) {  // 에러 발생 시 콜백 함수를 호출하면서 에러 객체 전달
          callback(err, null);
          return;
       }
       
       console.log('이름 [%s], 전화번호 [%s], 이메일 [%s]로 사용자 검색결과', name, phonenum, email);
       console.dir(results);
       
        if (results.length > 0) {  // 조회한 레코드가 있는 경우 콜백 함수를 호출하면서 조회 결과 전달
           console.log('이름 [%s], 전화번호 [%s], 이메일 [%s]이 일치하는 사용자 찾음.', name, phonenum, email);
           callback(null, results);
        } else {  // 조회한 레코드가 없는 경우 콜백 함수를 호출하면서 null, null 전달
           console.log("일치하는 사용자를 찾지 못함.");
           callback(null, null);
...
// 사용자 패스워드를 인증하는 함수
var findPw= function(database, id, email, phonenum, callback) {
   console.log('findPw 호출됨 : ' +id+', ' + email+ ', ' + phonenum);
   
    // 아이디와 비밀번호를 이용해 검색
   UserModel.find({"id":id, "email":email, "phonenum":phonenum}, function(err, results) {
      if (err) {  // 에러 발생 시 콜백 함수를 호출하면서 에러 객체 전달
         callback(err, null);
         return;
      }
      
      console.log('아이디 [%s], 이메일 [%s], 전화번호 [%s]로 사용자 검색결과', id,  email, phonenum);
      console.dir(results);
...
//비밀번호를 수정하는 함수
var savePw = function(database, id, password, passwordcheck, callback) {
   console.log('savePwd 호출됨 : '+ id + ', '+ password + ', ' + passwordcheck);
   
   var user = UserModel.update({"id":id},{$set:{"password":password,"passwordcheck":passwordcheck}});
...
//회원가입 함수
var addUser = function(database, id, password, passwordcheck, name,  phonenum, email, usertype, personalagree, callback) {
   console.log('addUser 호출됨 : ' + id + ', ' + password + ', ' + passwordcheck + ', ' + name +', '+phonenum +', '+email);
   
   // UserModel 인스턴스 생성
   var user = new UserModel({"id":id, "password":password, "passwordcheck":passwordcheck, "name":name, "phonenum":phonenum, "email":email, "usertype":usertype, "personalagree":personalagree});
...
//중복아이디 함수
var idCheck = function(database, id, callback) {
   ...
   UserModel.find({"id":id}, function(err, results) {
      ...
      console.dir(results);
       ...
       if (results.length > 0) {  // 조회한 레코드가 있는 경우 콜백 함수를 호출하면서 조회 결과 전달
          console.log('아이디[%s]이 일치하는 사용자 찾음.', id);
          callback(null, results);
       } else {  // 조회한 레코드가 없는 경우 콜백 함수를 호출하면서 null, null 전달
          console.log("일치하는 사용자를 찾지 못함.");
          callback(null, null);
